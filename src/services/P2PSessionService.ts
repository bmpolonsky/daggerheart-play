...
import { serverSessionEnabled, setActiveSessionTransportMode, shouldUseServerSession } from '../domain/p2p/serverSession';
...
export interface P2PSessionState {
  ...
  transportMode?: P2PSessionTransportMode;
  directPeers?: string[];
  rollPending?: boolean;
  pendingRollIntentId?: string | null;
}
...
export class P2PSessionService {
  ...
  private async openGmRoom(input: P2PSessionStartInput): Promise<void> {
    ...
    if (shouldUseServerSession('gm')) void this.saveCloudAssets();
    ...
    setActiveSessionTransportMode(transport.sessionMode());
    await this.activateMediaTransport(roomId, participant, transport);
    ...
    this.subscriptions.add(this.syncService.subscribePlayerRollIntents((message, _event, context) => {
      void (async () => {
        const accepted = this.applyPlayerRollIntent(message, remotePeerContext(context));
        this.patchSession({
    ...
    this.sessionStore.update((state) => {
      ...
      return {
        ...
        transportMode: transport.sessionMode(),
  ...
  private async openPlayerRoom(input: P2PSessionStartInput): Promise<void> {
    ...
    try {
      ...
      this.patchSession({
        ...
        routes: [],
        routePeers: [],
        message: error instanceof Error ? error.message : 'Не удалось подключиться к игре.'
      });
      throw error;
    }
    this.subscriptions.add(this.syncService.subscribePlayerRequests((request) => {
    ...
    this.subscriptions.add(this.syncService.subscribePlayerVoiceControls((message) => {
      ...
      if (message.type === 'forceMute' && peerMatches) {
        void this.mediaCallService?.muteMicrophone();
    ...
    this.sessionStore.update((state) => ({
      ...state,
      connected: true,
      status: this.playerTransportStatus(state.lastSnapshotAt),
      role: 'player',
      roomId,
      peerId: transport.peerId,
  ...
  async publishPlayerTokenMove(move: PlayerTokenMoveMessage): Promise<boolean> {
    const session = this.sessionStore.get();
    if (session.role !== 'player' || !session.connected || session.status !== 'connected') {
      return false;
    }
    try {
  ...
  async submitPlayerRollIntent(input: { actorId: string; actorName?: string; publication?: PlayerRollIntentMessage['publication']; intent: PlayerRollIntent; resourcePatch?: PlayerCharacterResourcePatch; teamworkEntryId?: string }): Promise<boolean> {
    ...
    if (session.rollPending) return false;
  ...
  private scheduleSnapshot(): void {
    const session = this.session$.get();
    const savesToServer = serverSessionEnabled() && Boolean(this.cloudBackupService);
    if (session.role !== 'gm' || (session.peers.length === 0 && !savesToServer)) {
      return;
    }
    window.clearTimeout(this.snapshotTimer);
  ...
  private createTransport(participant: TableParticipant): P2PRoomConnection {
    ...
    setActiveSessionTransportMode(adapter.sessionMode ?? 'p2p');
  ...
  private async activateMediaTransport(roomId: string, participant: TableParticipant, transport: P2PRoomConnection): Promise<void> {
    if (transport.sessionMode() !== 'hybrid' || !this.mediaTransportFactory) {
      this.sceneAudioBroadcastService?.setTransport(transport);
      this.mediaCallService?.setMediaTransport(transport);
      this.mediaTransportStore.set(initialMediaTransportState);
      return;
    ...
    const options: TrysteroP2PTransportOptions = {
      ...trysteroOptionsForNetworkSettings(readP2PNetworkSettings()),
      turnConfigProvider: turnConfigProvider(participant.id, true)
    };
    ...
    this.sceneAudioBroadcastService?.setTransport(mediaConnection);
    this.mediaCallService?.setMediaTransport(mediaConnection);
  }
  ...
  private handleRoomConnectionEvent(event: P2PRoomConnectionEvent): void {
    ...
    const transportMode = this.activeRoomConnection?.sessionMode?.() ?? 'p2p';
    const directPeers = this.activeRoomConnection?.directPeerIds?.() ?? [];
    if (event.type === 'ready') {
      const session = this.sessionStore.get();
      this.patchSession({ peerId: event.peerId, peers: event.peers, status: session.role === 'player' ? this.playerTransportStatus(session.lastSnapshotAt) : 'connected', routes, routePeers, transportMode, directPeers });
      return;
    }
    ...
    if (event.type === 'diagnostics-updated') {
      this.patchSession((state) => ({
        ...
        transportMode,
        directPeers,
        status: state.role === 'player' ? this.playerTransportStatus(state.lastSnapshotAt) : state.status
      }));
      return;
    }
