...
export function waitsForDiceReveal(event: PlayerViewModel['activity'][number]): boolean {
...
export function unrevealedRollIdsFromCompleted(
  current: Set<string>,
  completedDiceRollIds: Set<string>
): Set<string> | null {
  let next: Set<string> | null = null;
  for (const rollId of completedDiceRollIds) {
...
export function unrevealedRollIdsFromHistoricalActivity(
  ...
  for (const event of activity) {
    if (!waitsForDiceReveal(event) || !wasCreatedBefore(event.createdAt, mountedAtMs)) continue;
